...
- **Plain-language phase explainers**: Phase 2 and Phase 3 tabs each open with a short explainer paragraph naming the real actors in plain language ("Network Extender drones," "Medical Supply drone," "Heavy Payload drones") and how the two halves of each phase relate — this was previously left implicit in stats/timestamps alone, which read as unexplainable to a first-time viewer. The map legend was renamed to match ("Relay drone online," "Relief drone delivered"). Case Detail's Phase 1 damage-assessment mini map also moved from unlabeled blurred blobs (which just look like an abstract painting until you hover one) to always-visible per-station labels (3-letter code + severity, e.g. "MAN 6.2") plus a plain-language paragraph explaining what the panel represents and an "N ↑" orientation marker — nothing about it now depends on hovering to be understood.
- **Damage heat map**: a hand-rolled canvas heat layer (`createHeatCanvas` in `app.js`) — a real soft, full-coverage green→yellow→orange→red gradient wash, toggleable via the `Damage heat map` checkbox bottom-right, plus a miniature version (CSS-blurred blobs at each station's real relative position, with an always-visible per-station code+severity label) in Case Detail's Phase 1 tab — with an "Expand map" button opening the same visualization full-size (larger fonts/markers, and a horizontal-nudge collision fix for stations that sit close together geographically) for anyone actually trying to read it as an operator, not just glance at it. This simulates a computer-vision damage-assessment pass — there's no real imagery, so severity (0-10) comes directly from each station's own ("tower") raw physical sensor magnitude: rainfall + water-level rise for cloudburst, seismic amplitude for earthquake (`pipeline/damage.ts`). It's deliberately a separate metric from the detection probability above it — one answers "is this statistically anomalous," the other "how physically severe does it look" — not the same signal recolored.

  The renderer draws each station as a true canvas `createRadialGradient` on an offscreen alpha buffer (composited with `'lighter'` so overlapping stations sum smoothly, with no grid bucketing anywhere), then colorizes the accumulated alpha through the same gradient used for the legend — a hand-rolled implementation rather than a mapping-library heat plugin, so there's full control over exactly how intensity and overlap render at every zoom level.
...
- **Phase legibility**: Case Detail's stepper now shows a live one-line status under each phase label (e.g. "62% connectivity · 3/5 survivors found") instead of just a done/active/pending dot, computed the same way the tabs themselves are (`computePhase2`/`computePhase3` against the current playhead). A "Next: …" line above the stepper surfaces whichever real scheduled event — relay deploy, survivor found, medical delivery, resupply sortie — is coming up soonest (`nextMilestoneFor` in `app.js`), so you don't have to open a tab to see what's about to happen. The Cases list's compact phase-progress dots also carry a hover tooltip with the same detail per phase.
- **Accessibility & visual polish**: station risk status is now conveyed by shape as well as color — circle (normal), diamond (elevated), triangle (critical) — both in the station list and as ring style on the live map markers, so the classic red/green colorblind pair still reads correctly. All custom interactive controls (nav tabs, detail tabs, case rows, station rows, back link, timeline scrubber) are keyboard-operable (`Tab` + `Enter`/`Space`; arrow keys nudge the scrubber) with visible focus rings (`:focus-visible`), and `prefers-reduced-motion` disables/softens the pulsing map markers and modal/caption animations for anyone who has that OS preference set.
- **Impact reinforcement**: the Cases view has a live "what's actually been achieved" stat strip (survivors found, relay drones deployed, kg of relief delivered, cases fully resolved) aggregated across every revealed case — recomputed every render tick the same way the phase tabs are, never a static figure. Case Detail adds a per-case "impact snapshot" of real elapsed-time-since-detection metrics (time to first relay online, to full connectivity, to first survivor found, to first relief delivered) via `minutesBetween`/`formatDuration` in `app.js` — each stat stays "Pending" until the timeline has actually reached that real scheduled moment, so nothing is ever revealed early.
