...
- **Damage heat map**: a real soft/blurred radiating heat layer (the `leaflet.heat` plugin, not a grid of colored rectangles) on the Monitor map, toggleable via the `Damage heat map` checkbox bottom-right, plus a static miniature version (CSS-blurred blobs at each station's real relative position) in Case Detail's Phase 1 tab. This simulates a computer-vision damage-assessment pass — there's no real imagery, so severity (0-10, light red → heavy red) comes directly from each station's own ("tower") raw physical sensor magnitude: rainfall + water-level rise for cloudburst, seismic amplitude for earthquake (`pipeline/damage.ts`). It's deliberately a separate metric from the detection probability above it — one answers "is this statistically anomalous," the other "how physically severe does it look" — not the same signal recolored. Each station is fed into the heat layer as several overlapping points rather than one (`REPEAT` in `app.js`) — `leaflet.heat`'s single-point intensity turned out to be suppressed by an internal, zoom-dependent normalization undocumented in its API; stacking points exploits its point-accumulation behavior to get a predictable, tunable visual response instead of reverse-engineering that normalization further.
